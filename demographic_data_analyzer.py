...
def calculate_demographic_data(print_data=True):
    ...
    highest_country = df[df['salary'] == '>50K'].groupby('native-country')
    highest_earning = round(highest_country['salary'].count() / all_countries['salary'].count() * 100, 1).sort_values(ascending=False)

    highest_earning_country = highest_earning.index[0]
    highest_earning_country_percentage = highest_earning.max()

    # Identify the most popular occupation for those who earn >50K in India.
    highest_occupation = df[(df['salary'] == '>50K') & (df['native-country'] == 'India')].groupby('occupation')
    highest_india = highest_occupation['occupation'].count().sort_values(ascending=False)
